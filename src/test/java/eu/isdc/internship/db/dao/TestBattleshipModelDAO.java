...
public class TestBattleshipModelDAO {
	...
	public void initTests() {
		...
		posList.add(pos1);
		posList.add(pos2);
		pos1.setModel(shipModel);
		pos2.setModel(shipModel);
		...
		abList.add(ab1);
		abList.add(ab2);
		ab1.setModel(shipModel);
		ab2.setModel(shipModel);
		shipModel.setAv_BT(abList);
	...
	public void testAvailableBattleshipMapping() {
		...
		Assert.assertEquals(2, model.getAv_BT().size());
		abtDAO.delete(abtDAO.readAll().get(1));
		model = modelDAO.readAll().get(0);
		Assert.assertEquals(1, model.getAv_BT().size());
	}
	...
	public void testGetBattleshipModelsByIds() {
		...
		ids.add(results.get(0).getModel_id());
		...
		ids.clear();
		ids.add(results.get(0).getModel_id() + 1);
		results = modelDAO.getAllBattleshipModelsByIds(ids);
